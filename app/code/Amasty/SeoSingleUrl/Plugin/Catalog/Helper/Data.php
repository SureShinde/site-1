...
class Data
{
    ...
    public function aroundGetBreadcrumbPath(
        ...
        if ($type === Breadcrumb::CURRENT_URL && $product) {
			 ...
			 * The code below fails for products which are not assigned to any category:
			 * Magento generates an empty `WHERE` SQL clause: `WHERE (())`.
			 * To fix this bug, I added the `if ($urlArray) {...}` wrapper.
			 */
            if ($urlArray) {
