...
class Sitemap extends AbstractModel
{
    ...
    public function generateXml()
    {
        ...
        $this->_baseUrl = $this->_storeManager->getStore($this->_storeId)->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_LINK);
    ...
    protected function _generateProducts()
    {
        ...
        $productCollection->addAttributeToFilter('visibility', ['in' => [Visibility::VISIBILITY_BOTH, Visibility::VISIBILITY_IN_CATALOG, Visibility::VISIBILITY_IN_SEARCH]]);
        ...
        foreach ($productCollection as $product) {
            ...
            if ($this->getProductsThumbs()) {
                if ($image = $product->getImage()) {
                    $image = $this->_mediaConfig->getMediaUrl($image);
                }
                if ($image) {
                    $xmlLine .= '<image:image>';
                    if ($this->getProductsCaptions()) {
                        $label = $product->getThumbnailLabel();
                        if ($label == '') {
                            $label = $this->parsePlaceholder($product);
                        }
    ...
    {
        $this->_date = $this->_dateTime->gmtDate();
        $this->_baseUrl = $this->_storeManager->getStore()->getBaseUrl(\Magento\Framework\UrlInterface::URL_TYPE_WEB);
