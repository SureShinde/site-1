...
class Code extends \Magento\Framework\View\Element\Template
{
    ...
    /**
     * Returns custmer data needed for enhanced match.
     *
     * @return array
     */
    public function getCustomerData()
    {
        ...
        if ($isLoggedIn) {
            $customerId = $this->httpContext->getValue(
                \Apptrian\PinterestPixel\Model\Customer\Context
                    ::CONTEXT_CUSTOMER_ID
            );
        }
    ...
    /**
     * Returns category data needed for ads tracking.
     *
     * @return array
     */
    public function getCategoryData()
    {
        $c = $this->coreRegistry->registry('current_category');
        $name = $this->escapeSingleQuotes($c->getName());
        
        $data = [];
        
        $data['line_items'][0]['product_category'] = $name;
        
        return $data;
    }
    
    /**
     * Returns product data needed for ads tracking.
     *
     * @return array
     */
    public function getProductData()
    {
        ...
        $data['line_items'][0]['product_name'] = $this
            ->escapeSingleQuotes($p->getName());
        $data['line_items'][0]['product_id']   = $this
        ...
        foreach ($map as $parameter => $attribute) {
            ...
            if ($attributeValue) {
                $data['line_items'][0][$parameter] = $this
                    ->escapeSingleQuotes($attributeValue);
            }
        }
        
        // 'product_price' must be last element of the array
        // so everything is automatic in phtml file
        $data['line_items'][0]['product_price'] = $value;
    ...
    public function getOrderData()
    {
        ...
        if ($orderId) {
            ...
            foreach ($items as $item) {
                $name                         = $item->getName();
                $sku                          = $item->getSku();
                $products[$i]['product_name'] = $this
                    ->escapeSingleQuotes($name);
                $products[$i]['product_id']   = $this->escapeSingleQuotes($sku);
                ...
                // If product is configurable find parent name and SKU
                // and variant name and SKU
                if ($productType == 'configurable') {
                    $parentProduct = $this->getProductById(
                        $item->getProductId(),
                        $storeId
                    );
                    ...
                    $simpleSku = $pOptions['simple_sku'];
                    $simpleName = $pOptions['simple_name'];
                    
                    $products[$i]['product_name'] = $this
                        ->escapeSingleQuotes($parentName);
                    $products[$i]['product_id'] = $this
                        ->escapeSingleQuotes($parentSku);
                    $products[$i]['product_variant_id'] = $this
                        ->escapeSingleQuotes($simpleSku);
                    
                    if ($parentName != $simpleName) {
                        $products[$i]['product_variant'] = $this
                            ->escapeSingleQuotes($simpleName);
                    }
                }
                
                // If product is bundle find parent name and SKU
                // and variant SKU
                if ($productType == 'bundle') {
                    ...
                    $products[$i]['product_id'] = $this
                        ->escapeSingleQuotes($parentSku);
                    $products[$i]['product_variant_id'] = $this
                        ->escapeSingleQuotes($sku);
                }
                
                $productQuantity = round($item->getQtyOrdered(), 0);
                $products[$i]['product_quantity'] = $productQuantity;
                $orderQuantity += $productQuantity;
                
                $price = $item->getPrice();
                // 'product_price' must be last element of the array
                $products[$i]['product_price'] = $this->formatPrice($price);
    ...
    public function getParameterToAttributeMap($type = 'product')
    {
        ...
        $data = $this->getConfig(
            'apptrian_pinterestpixel/general/mapping_' . $type,
            $this->getStoreId()
        );
