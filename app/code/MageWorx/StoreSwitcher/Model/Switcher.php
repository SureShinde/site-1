...
class Switcher
{
    ...
    {
        /** @var \Magento\Store\Api\Data\StoreInterface|\Magento\Store\Model\Store $storeModel */
        $storeModel = $this->storeManager->getStore($customerStoreCode);
        $params = $this->resolveParamsForRedirect();
        
        if ($this->request->getFullActionName() == 'catalog_product_view' && $this->request->getParam('id')) {
            /** @var \Magento\Catalog\Model\Product $product */
            $product = $this->productRepository->getById($this->request->getParam('id'));
            $redirectUrl = $this->productUrl->getUrl($product, $params);
        } else {
            $redirectUrl = $storeModel->getUrl($this->request->getBasePath(), $params);
        }
    ...
    /**
     * Resolve url params for redirect url
     *
     * @return array
     */
    private function resolveParamsForRedirect()
    {
        $params = [];
        $geoipCountry = $this->request->getParam('geoip_country');
        if ($geoipCountry) {
            $params['_query'] = 'geoip_country=' . $geoipCountry;
        }
